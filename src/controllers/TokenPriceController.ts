...
export class TokenPriceController {
    private coimarket = new CoinMarketCap();
    ...
    private getAllTokensPricesInUSD() {
        return new BluebirbPromise((resolve, reject) => {
            this.coimarket.getTicker({limit: 0, structure: "array"}).then((prices: any) => {
                resolve(prices.data);
            }).catch((error: Error) => {
                reject(error);
            });
    ...
    private async getCoefficient(currency: string): Promise<number> {
        try {
            const prices = await this.coimarket.getTicker({limit: 1, convert: currency, structure: "array"})
